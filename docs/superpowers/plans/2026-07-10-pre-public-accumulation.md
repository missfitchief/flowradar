...
## PHASE 6a — CAPITAL LINEAGE ROOTS: dynamic import + permanent monitoring subscriptions (PULLED FORWARD, operator directive 2026-07-10)
Operator supplies root-wallet text files (any size — N is determined by the parser, never hardcoded; the first fixture's count is input data, not a product constraint). Pure parser in @flowradar/core (`parseRootWalletFile`): one address per line, strips blank lines + `#`/`|` comments, preserves inline labels, validates base58→32-byte Solana pubkeys, parks EVM `0x` rows, dedupes; returns dynamic counts (total/valid/evm/duplicates/malformed/labels). DB importer (`importRootWallets`): idempotent + incremental — new wallets minted observation_only (NEVER signal_eligible; no stats rows — nothing is claimed), existing wallets' status/classification/notes untouched; per root one permanent `LineageRoot` record + one `MonitoringSubscription` (priority root_permanent) upserted; roots absent from later files are never auto-deleted; configurable operational safety limit only (LINEAGE_IMPORT_MAX_ROOTS, default 10000). Scheduler contract: NO per-root timers — subscriptions are consumed in priority-ordered batches with cursors under provider rate budgets (walletActivity remains the baseline poller; observation_only is always polled since Phase 0). Generic tests: 1 root, 100+ roots, dedupe, incremental second import adds only new, re-import preserves classification+subscriptions, no fixed-count assumption in production code. STOP after import report — historical graph expansion (receiver rule, WalletRelationship, weak-link lifecycle) needs separate approval.
