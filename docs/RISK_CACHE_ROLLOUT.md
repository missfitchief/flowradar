...
2. **Pre-warm the cache BEFORE consumers read it:** run `npx tsx scripts/prewarm-risk-cache.mts`. The cutover gate is the script's **`readyForCutover: true`** — defined as *usable-observation coverage*: every SOLANA traded token has a snapshot with `observedAt != null` (a real observation, or an honest provider-returned `unavailable`, which is exactly what the direct path yields for that mint). An empty due-queue is **NOT** completion — tokens whose only attempts failed (throttled/error, `observedAt` null) are still not-ready while backing off, and the script waits out those windows rather than declaring success (Codex final-gate fix). The script exits non-zero on any not-ready end (3 = sustained throttling, 4 = other), so the resume step mechanically cannot proceed on a cold cache. Watch: `refreshed` climbs, `throttled` stays low (Retry-After backoff working), `errors` ≈ 0.
